...
Runs the same live v2 LLM runtime smoke as check-live-scheduler.sh but explicitly
asserts that the single visible/selected GPU path can materialize naim/* runtime
containers through hostd exec mode.
EOF
}
...
work_root="$(mktemp -d "${repo_root}/var/live-single-gpu-v2.XXXXXX")"
db_path="${work_root}/controller.sqlite"
artifacts_root="${work_root}/artifacts"
runtime_root="${work_root}/runtime"
state_root="${work_root}/hostd-state"
state_path="${work_root}/single-gpu-v2.desired-state.v2.json"
plane_name="single-gpu-v2"
...
echo "[live-single-gpu] deploying v2 llm plane"
...
echo "[live-single-gpu] waiting for live runtime observation"
for _ in $(seq 1 180); do
  ...
  if printf '%s' "${observations}" | grep -F 'applied_generation=1' >/dev/null && \
     printf '%s' "${observations}" | grep -F 'gpu device=0' >/dev/null; then
    break
  fi
  sleep 2
done
"${build_dir}/naim-controller" show-host-observations --db "${db_path}" --node local-hostd | grep -F 'applied_generation=1' >/dev/null
"${build_dir}/naim-controller" show-host-observations --db "${db_path}" --node local-hostd | grep -F 'gpu device=0' >/dev/null
