...
HttpResponse PlaneHttpService::HandlePlanesCollection(
  ...
  if (request.method == "GET") {
    try {
      return deps_.build_json_response(200, deps_.build_planes_payload(db_path), {});
...
HttpResponse PlaneHttpService::HandlePlanePath(
  ...
  if (request.method == "GET" && remainder.find('/') == std::string::npos) {
    try {
      return deps_.build_json_response(
          200,
          deps_.build_controller_state_payload(db_path, remainder),
          {});
    } catch (const std::exception& error) {
      return deps_.build_json_response(
          500,
...
HttpResponse PlaneHttpService::HandleControllerState(
  ...
  try {
    return deps_.build_json_response(
        200,
        deps_.build_controller_state_payload(db_path, std::nullopt),
        {});
  } catch (const std::exception& error) {
    return deps_.build_json_response(
        500,
