...
std::optional<HttpResponse> ModelLibraryHttpService::HandleRequest(
  ...
  if (request.path == "/api/v1/model-library/quantize") {
    if (request.method != "POST") {
      return support_.build_json_response(
          405, json{{"status", "method_not_allowed"}}, {});
    }
    try {
      return support_.model_library_service().EnqueueQuantization(db_path, request);
    } catch (const std::exception& error) {
      return support_.build_json_response(
          500,
          json{{"status", "internal_error"},
  ...
  if (request.path == "/api/v1/model-library/jobs/stop") {
    ...
    try {
      return support_.model_library_service().StopDownloadJob(db_path, request);
    } catch (const std::exception& error) {
      return support_.build_json_response(
          500,
          json{{"status", "internal_error"},
  ...
  if (request.path == "/api/v1/model-library/jobs/resume") {
    if (request.method != "POST") {
      return support_.build_json_response(
          405, json{{"status", "method_not_allowed"}}, {});
    }
    try {
      return support_.model_library_service().ResumeDownloadJob(db_path, request);
    } catch (const std::exception& error) {
      return support_.build_json_response(
          500,
          json{{"status", "internal_error"},
  ...
  if (request.path == "/api/v1/model-library/jobs") {
    ...
    try {
      return support_.model_library_service().DeleteDownloadJob(db_path, request);
    } catch (const std::exception& error) {
      return support_.build_json_response(
          500,
          json{{"status", "internal_error"},
