...
std::string JoinRoutes(const RouteSummary& routes) {
  std::string joined;
...
std::string BuildControllerRoutesSummary(bool webgateway_routes_enabled) {
  RouteSummary routes;
...
std::string BuildSkillsFactoryRoutesSummary() {
  RouteSummary routes;
  AppendRoutes(routes, {
      "/health",
      "/api/v1/health",
...
int ServeControllerHttp(
  ...
  return server.Serve({
      ...
      listen_port,
      ui_root,
      BuildControllerRoutesSummary(webgateway_routes_enabled),
  });
}
...
int ServeSkillsFactoryHttp(
  ...
  return server.Serve({
      ...
      listen_port,
      std::nullopt,
      BuildSkillsFactoryRoutesSummary(),
  });
}
